...
## 7. [v1.0.5.Build.6] 추론 렌더링 타임아웃 무제한(Unlimited) 전환
- **진단 및 이슈**: 대형 비디오 모델(MiniMax H3)의 최초 가중치 로딩 및 장시간 추론 시 30분 타임아웃으로 인한 중단 우려 해소 요구.
- **해결**:
  1. `config.py`에 `rendering_timeout_sec: int = 0` (무제한) 전역 설정 적용.
  2. `comfy_workflow_engine.py`: `poll_execution`에서 `timeout_sec <= 0` 또는 `None`일 때 시간 제한 없이 100% 완료 시까지 안정적으로 대기하도록 개편.
  3. 사용자가 수동 중단을 원할 경우 UI의 `[작업 중단]` 버튼을 통해 언제든지 즉시 중단 가능.

---

## 8. [v1.0.6.Build.7] 대기열 다중 작업 순차 일괄 처리 및 작업 완료 후 PC 자동 종료 예약
- **진단 및 이슈**: 장시간 소요되는 다중 렌더링 작업을 대기열에 걸어두고 외출/수면 시 모든 작업이 완료된 후 PC를 자동으로 안전하게 종료하는 기능 탑재 요구.
- **해결**:
  1. `task_queue_manager.py`: `_worker_loop`의 순차 배치 스케줄러를 기반으로 대기열의 모든 PENDING 작업이 마감되었을 때 Windows `shutdown /s /t 60` 호출 로직 연동.
  2. `app_gui.py`: 작업 대장 상단에 `[x] 전체 작업 완료 시 PC 종료 예약` 체크박스 배치.
  3. 완료 시 60초 시각적 카운트다운 팝업 모달 표출 및 `[PC 종료 즉시 취소]`(`shutdown /a`) 원클릭 안전 방어막 완비.
